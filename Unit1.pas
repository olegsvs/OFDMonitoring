...
uses
  ...
  Registry, ComCtrls, ImgList, ComObj, Graphics, Menus, Shellapi, ToolWin, DrvFRLib_TLB;
...
begin
  ...
  try
    ...
    tmpString :=('Попытка запроса данных с ' + IP + ':' + IntToStr(port) + ' ' + DateToStr(Now) +' '+ TimeToStr(Now));
    Synchronize(test);
    if Drv.Connect <> 0 then // Проверяем подключение
      ...
      Synchronize(test);
      tmpString :=('Отправление предупреждения на ' + email +'(запуск mail_con.vbs)');
      Synchronize(test);
        if(CheckBox2.checked = true) then
          begin
            tmpString :=('Отправка сообщений включена');
            Synchronize(test);
            ShellExecute(0, nil, 'mail_con.vbs', PChar(ip + ' ' + email), nil, SW_SHOW);
          end  else
            begin
    ...
    begin
      ...
      begin
          ...
          if(docs = 0) then RichTDK.SelAttributes.Color:=clGreen;
          if(docs > 0) then RichTDK.SelAttributes.Color:=clPurple;
          if(docs > 200) then RichTDK.SelAttributes.Color:=clRed;
          tmpString :=('Неотправленных документов: ' + IntToStr(docs));
          Synchronize(test);
          if(Drv.ValueOfFieldInteger > 100) then
          begin
            RichTDK.SelAttributes.Color:=clRed;
            tmpString :=('Отправление предупреждения на ' + email +'(запуск mail.vbs)');
            Synchronize(test);
            if(CheckBox.checked = true) then begin
            tmpString :=('Отправка сообщений включена');
            Synchronize(test);
            ShellExecute(0, nil, 'mail.vbs', PChar(ip + ' ' + IntToStr(Drv.ValueOfFieldInteger) + ' ' + email), nil, SW_SHOW);
            end else begin
            tmpString :=('Отправка сообщений отключена');
            Synchronize(test);
          end;
      ...
      begin
        RichTDK.SelAttributes.Color:=clRed;
        tmpString :=('Ошибка Driver.FieldType код: ' + Drv.ResultCodeDescription);
        Synchronize(test);
      end;
      end else
        begin
          RichTDK.SelAttributes.Color:=clRed;
          tmpString :=('Ошибка Driver.ReadTable код: ' + Drv.ResultCodeDescription);
          Synchronize(test);
        end;
  finally
    tmpString :=('');
